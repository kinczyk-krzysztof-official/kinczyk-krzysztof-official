...
- **Three wrong root causes before the real one (CS30, most recent)** — a recurring device side-effect was misdiagnosed three separate times across three sessions before a systematic, cross-brand investigation traced it to a documented AOSP tool behavior, confirmed experimentally on two phone manufacturers. The case study is as much about the failure pattern (repeated wrong attribution) as the eventual correct answer.
